...
export function SignupForm({role}) {
  ...
  async function onSubmit(event) {
    ...
    try {
      ...
      const response = await fetch("/api/register", { //บันทึกข้อมุล
        method: "POST",
        headers: {
        ...
        <div className="grid gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <Input id="first-name" name="first-name" placeholder="SK" required />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="last-name">Last name</Label>
              <Input id="last-name" name="last-name"  placeholder="JD" required />
            </div>
          </div>
          <div className="grid gap-2">
